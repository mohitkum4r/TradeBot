...
class PairsTradingStrategy(BaseStrategy):
    ...
    def generate_signal(
        ...
        """
        Note: The 'data' for this strategy must contain the close prices
        of BOTH stocks in the pair.
        """
        if self.stock1 not in data.columns or self.stock2 not in data.columns:
            return "HOLD", "Data for one or both stocks in the pair is missing."
        ...
        if p_value > 0.05:
            return "HOLD", f"Pair is not cointegrated (p-value: {p_value:.4f})."
        ...
        if abs(last_z) < self.exit_z:
            return "EXIT", f"Z-score ({last_z:.2f}) reverted to mean. Close position."

        return "HOLD", f"Z-score ({last_z:.2f}) is within entry/exit thresholds."
